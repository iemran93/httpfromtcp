...
func TestHeaderParse(t *testing.T) {
	...
	// Test: Duplicated Headers & case insensitive headers
	reader = &chunkReader{
		data:            "GET / HTTP/1.1\r\nHost: local\r\nhost: very-local\r\n\r\n",
		numBytesPerRead: 3,
	}
	...
	require.NoError(t, err)
	require.NotNil(t, r)
	assert.Equal(t, "local, very-local", r.Headers["host"])

	// Tset: Missing End of Headers
	reader = &chunkReader{
		data:            "GET / HTTP/1.1\r\nHost: local\r\nhost: very-local\r\n",
		numBytesPerRead: 3,
	}
...
func TestBodyParse(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, r)
	assert.Equal(t, "pa", string(r.Body))
}
